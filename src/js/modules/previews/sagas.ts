...
import { Api, ApiEntity, ApiEntityResponse, ApiPreviewResponse } from '../../api/types';
...
export default function createSagas(api: Api) {
  function* fetchPreview(
    ...
    if (response) {
      ...
      const deployment: Deployment[] = yield call(toDeployments, response.deployment);
  ...
  return {
    sagas: [
      takeEvery(LOAD_PREVIEW_AND_COMMENTS, loadPreviewAndComments),
    ],
  };
}
